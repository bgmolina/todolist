...
while true; do
  ...
  select opt in "${OPTIONS[@]}"; do
    ...
    "3")
      ...
      # check if pod with label app=todolist and namespace=web is ready when deploying and
      # forward port 3000 to 80 (server) for testing purposes only (not for production)
      # wait for 60 seconds.
      microk8s kubectl wait --for=condition=ready -n "$NAMESPACE" pod -l app=todolist --timeout=60s &&
      microk8s kubectl port-forward service/todolist-svc 3000:80 -n "$NAMESPACE"
